...
export default function App() {
  ...
  async function handleAddWatched(movie) {
    ...
    const { imdbID, imdbRating, title, year, poster, runtime, userRating } =
      movie;
    try {
      const res = await fetch(
        `http://localhost:4000/api/movie/addmovie/${user._id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async function handleDeleteWatched(id) {
    try {
      ...
      const resdata = await res.json();
      console.log(resdata);
      if (resdata.status === "success") toast.success("Movie Deleted");
    ...
    async function getMymovies() {
      if (!user || !user._id || user._id !== "") {
        try {
          ...
          const res = await fetch(
            `http://localhost:4000/api/movie/mymovies/${user._id}`,
            {
              method: "GET",
              headers: {
                "Content-Type": "application/json",
          ...
          const resdata = await res.json();
          console.log(resdata);
          if (resdata.status === "success") setWatched(resdata.movies);
        } catch (e) {
          // toast.error(e.message);
        }
      }
  ...
  return (
    <>
      ...
      <Main>
        <Box>
          {isTrending ? (
            <h1 className="headings">Trending Movies</h1>
          ) : (
            <h1 className="headings">Matched Movies</h1>
          )}
          {isLoading && <Loader />}
          {!isLoading && !error && !isTrending && (
  ...
  if (isTrending) {
    const genres = [];
    {
      trendingMovie.genres &&
        trendingMovie.genres.forEach((element) => {
    ...
    return (
      <div className="details">
        {isLoading ? (
          ...
          <>
            <header>
              <button className="btn-back" onClick={oncCloseMovie}>
                ❎
              </button>
              <img
              ...
              <div className="details-overview">
                ...
                <p>
                  {trendingMovie.release_date} &bull; {trendingMovie.runtime}
                </p>
                <p>{genres.join(",")}</p>
                <p>
                  <span>⭐️</span>
                  {trendingMovie.vote_average} IMdb rating
                </p>
              </div>
            </header>
            <section>
